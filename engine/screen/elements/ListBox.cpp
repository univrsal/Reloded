...
ListBox::ListBox(uint8_t id, int x, int y, int w, int max_items, Screen *parent)
{
    ...
    m_scroll_bar = new Scrollbar(id, x + w - 12, y + 2, temp.h - 4, get_max_scroll(), SCROLL_TYPE_DIALOG, parent);
...
void ListBox::handle_events(SDL_Event *e)
{
    ...
    if (e->type == SDL_MOUSEMOTION)
    {
        if (is_mouse_over(e->button.x, e->button.y))
            m_mouse_over_item = (uint16_t) SDL_min((e->button.y - get_dimensions()->y) / m_item_height, m_max_items);
        else
            m_mouse_over_item = (uint16_t) -1;
    }
    ...
    else if (e->type == SDL_KEYDOWN)
    {
        ...
        {
            case SDLK_UP:
                select_item((uint16_t) (m_selected_item - 1));
            case SDLK_PAGEUP:
            ...
            case SDLK_END:
                ...
                select_item((uint16_t) (get_item_count() - 1));
            default:
                break;
        }
    }
...
{
    return (uint16_t) SDL_max(0, SDL_min(get_item_count() - 1, m_selected_item));
}

uint16_t ListBox::get_item_count(void)
{
    return (uint16_t) (int) m_list_items.size();
}

void ListBox::select_item(uint16_t i)
{
    m_selected_item = (uint16_t) SDL_max(0, SDL_min(get_item_count() - 1, i));
}

SDL_Rect ListBox::get_dim_for_item(uint16_t i)
{
    return SDL_Rect{get_dimensions()->x + 3, get_dimensions()->y + 2 + (m_item_height * i), m_item_width,
                    m_item_height};
}
...
uint16_t ListBox::get_max_scroll(void)
{
    if (is_empty())
        return 0;
    return (uint16_t) SDL_max(get_item_count() - m_max_items, 0);
}
