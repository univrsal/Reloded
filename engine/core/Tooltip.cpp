...
Tooltip::Tooltip(Screen *parent_screen)
{
	...
	m_bg_color = m_parent_screen->m_resources->palette()->get_bg();
    m_is_visible = false;
    m_line_height = 0;
}
...
void Tooltip::set_pos(int x, int y)
{
    ...
    SDL_GetWindowSize(m_parent_screen->m_resources->window(), &window_w, &window_h);
...
void Tooltip::set_text(std::string *text)
{
    ...
    while (end != std::string::npos) {
        ...
        if (!token.empty())
            dim = m_parent_screen->m_resources->renderer()->util_text_default_dim(&token);
